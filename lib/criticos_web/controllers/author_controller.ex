defmodule CriticosWeb.AuthorController do
  ...
  def edit(%{assigns: %{current_user: user}} = conn, %{"id" => id}) do
    ...
    case Library.get_author!(id) do
      %{creator_id: ^creator_id} = author ->
        changeset = Library.change_author(author)
        render(conn, :edit, author: author, changeset: changeset)

      _ ->
        conn
        |> put_flash(:error, "You do not have permissions to modify this resource.")
